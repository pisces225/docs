...
It is recommended to deploy the DM cluster using DM-Ansible. For detailed deployment, see [Deploy Data Migration Using DM-Ansible](/tools/dm/deploy-using-ansible.md).

> **Notes**:
> 
> - For database passwords in all the DM configuration files, use the passwords encrypted by `dmctl`. If a database password is empty, it is unnecessary to encrypt it. See [Encrypt the upstream MySQL user password using dmctl](/tools/dm/deploy-using-ansible.md#encrypt-the-upstream-mysql-user-password-using-dmctl).
