...
For database related passwords in all the DM configuration files, use the passwords encrypted by `dmctl`. If a database password is empty, it is unnecessary to encrypt it. For how to encrypt the plaintext password, see [Encrypt the upstream MySQL user password using dmctl](/tools/dm/deploy-using-ansible.md#encrypt-the-upstream-mysql-user-password-using-dmctl).
...
2. Use Ansible to [stop the entire DM cluster](/tools/dm/deploy-using-ansible.md#step-10-stop-the-dm-cluster).
...
5. Use Ansible to [start the entire DM cluster](/tools/dm/deploy-using-ansible.md#step-9-deploy-the-dm-cluster).
